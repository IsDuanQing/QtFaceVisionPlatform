...
当前主线是人脸检测 / 识别平台。本文中音频、TensorRT 和工业缺陷检测相关内容属于历史排查记录；当前阶段暂不接音频，优先处理视频、人脸检测、人脸识别、存储和事件链路。
...
说明：这是历史问题记录。当前人脸平台主线暂不接音频，`modules/audio` 不作为下一阶段开发重点。
...
RTSP 输入当前仍然暂时禁用音频，因为人脸平台主流程优先关注视频帧、检测结果、识别结果和实时性。

## 3. 本地视频音画不同步

说明：这是历史问题记录。当前人脸平台主线以视频帧、检测框、识别结果和事件为核心，暂不继续推进音频同步。
...
## 7. OpenCV DNN 报 reshape / outTotal == inpTotal
...
这类问题通常不是 UI 绘制错误，而是 ONNX 模型和输入配置不匹配。常见原因：

1. 仍然使用旧缺陷检测模型路径。
2. 仍然保留旧模型的 `1088 x 1088` 输入尺寸。
3. labels 文件和 ONNX 文件不是同一套模型。
4. 类别数和 labels 文件不一致。
5. 当前 OpenCV DNN 版本不支持该 ONNX 中的某些层或动态 reshape。
...
Qt 界面里优先点击 Parameters 中的 `Face` 预设，再打开视频或 RTSP。

当前代码在 `YoloOpenCVDnnDetector::initialize()` 中增加了输入形状验证和一次空输入 forward 探测。这样模型 / 输入形状不兼容时，会尽量在检测器初始化阶段给出清晰错误。

对应代码：

- `modules/inference/src/YoloOpenCVDnnDetector.cpp`
- `apps/viewer/ViewerSettingsStore.cpp`
- `apps/viewer/MainWindow.cpp` 的 `applyFaceDetectorPreset()`

## 8. 添加人员后 Recognition 不是 Ready

### 现象

Faces 页签可以添加人员，但 `Recognition` 状态显示：

```text
Unavailable
```

或者模板数一直是 `0 templates`。

### 原因分析

真实识别闭环比“保存人员信息”多了几个步骤：
...
1. 参考图路径不存在。
2. 参考图中没有检测到人脸。
3. 当前 OpenCV DNN 检测器没有正确初始化。
4. OpenCV face 模块没有启用，特征提取不可用。
5. 参考图质量太差，裁剪后的人脸太小。

### 解决方案

1. 先确认 Parameters 中的 `Face` 预设可正常检测视频人脸。
2. 使用清晰单人参考图。
3. 添加人员后查看 Faces 页签中的 `Recognition` 状态和 tooltip。
4. 如果特征提取失败，当前代码不会覆盖已有模板，避免误删旧特征。

对应代码：

- `modules/recognition/src/FaceRecognizer.cpp`
- `apps/viewer/MainWindow.cpp` 的 `addFaceIdentity()`
- `apps/viewer/MainWindow.cpp` 的 `updateFaceRecognitionDiagnostics()`

## 9. 同一个人连续出现时事件太多

### 现象

同一个人在视频里连续出现，多帧都能识别成功。如果每帧都生成事件，后续告警和事件列表会被刷屏。
...
- 检测记录应该尽量完整保存，方便历史追溯。
- 识别事件应该按业务节奏去重，方便告警和审计。

### 解决方案

当前 `SQLiteDetectionStorage` 优先按轨迹对识别事件去重：
...
如果结果没有 `track_id`，才回退到旧的 5 秒冷却策略。

这不会删除检测记录，也不会删除 `detection_face_links`。History 仍然可以显示连续帧的识别结果。

对应代码：

- `modules/storage/src/SQLiteDetectionStorage.cpp`
- `SQLiteDetectionStorage::shouldInsertRecognitionEventLocked()`
- `tests/storage/SQLiteDetectionStorageTest.cpp`

## 后续建议

下一步应该继续把显示链路、推理链路、识别链路和存储链路拆开：
...
FrameDispatcher
  ...
  |
  |-----------------> InferenceQueue -> OpenCV DNN 人脸检测
                                      -> FaceRecognizer
                                      -> StorageQueue
```
...
3. UI 线程不能承担重计算。
4. 人脸平台中，检测记录和识别事件应该分开建模。
5. 后续应把 SQLite 写入迁移到后台线程。

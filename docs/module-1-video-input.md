...
本模块的目标，是把视频层从“服务 Qt 显示的辅助代码”，重构成后续工业视觉检测平台可以复用的视频输入组件。
...
这样做的好处是：解码器不再知道 Qt 界面怎么显示图片，后续接 OpenCV、TensorRT、网络服务时也不会被 Qt 代码绑住。
...
5. 如果 TensorRT 后续需要 BGR、RGB planar，或者归一化后的 float 输入，这个模块应该怎么扩展？
...
`FFmpegDecoder::open(VideoInputConfig)` 是后续接 RTSP、工业相机、模拟数据源的统一入口。
